...
export default function Indicadores(){
 const report=useQuery({queryKey:['clinical-indicators'],queryFn:async()=>{
  const [patients,appointments,waiting,encounters]=await Promise.all([
  ...
  ]);for(const r of [patients,appointments,waiting,encounters])if(r.error)throw r.error;return{patients:patients.count??0,appointments:appointments.count??0,waiting:waiting.count??0,encounters:encounters.count??0};
 }});
