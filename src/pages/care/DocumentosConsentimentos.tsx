...
export default function DocumentosConsentimentos() {
  ...
  const upload = useMutation({ mutationFn: async () => {
    ...
    const { data: auth } = await supabase.auth.getUser();
    const { error } = await (supabase.from('documents') as any).insert({ organization_id: patient.organization_id, patient_id: patientId, document_type: documentType, file_name: file.name, mime_type: file.type || 'application/octet-stream', storage_path: path, file_size: file.size, created_by: auth.user?.id });
    if (error) throw error;
  }, onSuccess: () => { refresh(); setFile(null); setDocumentType('attachment'); toast({ title: 'Documento privado anexado' }); }, onError: (error: Error) => toast({ title: 'Não foi possível anexar', description: error.message, variant: 'destructive' }) });
