...
export default function DocumentosConsentimentos() {
  ...
  const data = useQuery({ queryKey: ['clinical-documents'], queryFn: async () => {
    const [patients, consents, documents] = await Promise.all([
      ...
      supabase.from('consents').select('id,patient_id,consent_type,status,granted_at,created_at,patient:patients(record_number,person:persons(full_name))').order('created_at', { ascending: false }).limit(40),
      supabase.from('documents').select('id,patient_id,document_type,file_name,mime_type,file_size,created_at,patient:patients(record_number,person:persons(full_name))').is('archived_at', null).order('created_at', { ascending: false }).limit(40),
    ]);
  ...
  const grantConsent = useMutation({ mutationFn: async () => {
    ...
    const { error } = await (supabase.from('consents') as any).insert({ organization_id: patient.organization_id, patient_id: patientId, consent_type: consentType, status: 'granted', granted_at: new Date().toISOString(), created_by: auth.user?.id });
  ...
  const upload = useMutation({ mutationFn: async () => {
    ...
    const { error: storageError } = await supabase.storage.from('clinical-documents').upload(path, file, { contentType: file.type, upsert: false });
  ...
  return <MainLayout><div className="space-y-6"><div className="flex gap-3"><div className="rounded-lg bg-primary/10 p-2"><FileCheck2 className="h-6 w-6 text-primary" /></div><div><h1 className="text-2xl font-bold">Documentos e consentimentos</h1><p className="text-sm text-muted-foreground">Anexos ficam em armazenamento privado; o sistema registra somente metadados e trilha clínica.</p></div></div><div className="grid gap-5 lg:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">Consentimento</CardTitle><CardDescription>Registre a versão e a concessão do paciente.</CardDescription></CardHeader><CardContent><form onSubmit={submit(() => grantConsent.mutate())} className="space-y-3"><PatientPicker patients={patients} value={patientId} onChange={setPatientId}/><div className="space-y-1"><Label>Tipo</Label><Input value={consentType} onChange={(event) => setConsentType(event.target.value)} required /></div><Button disabled={!patientId || grantConsent.isPending}><ShieldCheck className="mr-1 h-4 w-4" />Conceder consentimento</Button></form></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Anexo clínico</CardTitle><CardDescription>O arquivo é enviado para o bucket privado da organização.</CardDescription></CardHeader><CardContent><form onSubmit={submit(() => upload.mutate())} className="space-y-3"><PatientPicker patients={patients} value={patientId} onChange={setPatientId}/><div className="space-y-1"><Label>Arquivo</Label><Input type="file" onChange={(event: ChangeEvent<HTMLInputElement>) => setFile(event.target.files?.[0] ?? null)} required /></div><Button disabled={!patientId || !file || upload.isPending}><FileUp className="mr-1 h-4 w-4" />Anexar documento</Button></form></CardContent></Card></div><Card><CardHeader><CardTitle className="text-base">Consentimentos recentes</CardTitle></CardHeader><CardContent className="space-y-2">{(data.data?.consents as any[])?.length ? (data.data?.consents as any[]).map((item) => <div key={item.id} className="flex items-center justify-between rounded border p-3 text-sm"><span>{item.patient?.person?.full_name ?? item.patient?.record_number} · {item.consent_type}</span><Badge>{item.status}</Badge></div>) : <p className="text-sm text-muted-foreground">Nenhum consentimento registrado.</p>}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Documentos recentes</CardTitle></CardHeader><CardContent className="space-y-2">{(data.data?.documents as any[])?.length ? (data.data?.documents as any[]).map((item) => <div key={item.id} className="flex items-center justify-between rounded border p-3 text-sm"><span>{item.file_name} · {item.patient?.person?.full_name ?? item.patient?.record_number}</span><span className="text-muted-foreground">{new Date(item.created_at).toLocaleString('pt-BR')}</span></div>) : <p className="text-sm text-muted-foreground">Nenhum documento anexado.</p>}</CardContent></Card></div></MainLayout>;
}

function PatientPicker({ patients, value, onChange }: { patients: any[]; value: string; onChange: (value: string) => void }) { return <div className="space-y-1"><Label>Paciente</Label><Select value={value} onValueChange={onChange}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent>{patients.map((item) => <SelectItem key={item.id} value={item.id}>{item.record_number} — {item.person?.full_name ?? ''}</SelectItem>)}</SelectContent></Select></div>; }
