...
export default function Veterinaria() {
  ...
  const [taskHospitalizationId, setTaskHospitalizationId] = useState("");
    ...
    queryFn: async () => {
      const [
        ...
        (supabase as any)
          .from("animals")
          .select(
            "id,clinic_id,name,species,breed,sex,microchip_number,animal_guardians(person:persons(full_name))",
          )
          .is("archived_at", null)
          .order("name"),
        (supabase as any)
        ...
        (supabase as any)
          ...
          .select("id,animal_id,chief_complaint,diagnosis,created_at")
          .order("created_at", { ascending: false })
          .limit(12),
        (supabase as any)
          .from("veterinary_weights")
          .select("id,animal_id,weight_kg,measured_at")
          .order("measured_at", { ascending: false })
          .limit(12),
        (supabase as any)
          .from("veterinary_vaccinations")
          .select("id,animal_id,vaccine_name,administered_at,next_due_at")
          .order("administered_at", { ascending: false })
          .limit(12),
        (supabase as any)
          .from("veterinary_hospitalizations")
          .select("id,animal_id,box_code,status,admission_reason,admitted_at")
          .in("status", ["admitted"])
          .order("admitted_at", { ascending: false }),
        (supabase as any)
          .from("veterinary_hospitalization_tasks")
          .select("id,hospitalization_id,title,task_type,scheduled_at,status")
          .in("status", ["planned"])
          .order("scheduled_at")
          .limit(30),
      ]);
      ...
      return {
        ...
        hospitalizationTasks: hospitalizationTasks.data ?? [],
      };
    },
  });
  const animalClinic = (animalId: string) => {
    const animal = ((data.data?.animals ?? []) as any[]).find(
      (item) => item.id === animalId,
    );
    const clinic = (data.data?.clinics ?? []).find(
      (item) => item.id === animal?.clinic_id,
    );
  ...
  const createWeight = useMutation({
    ...
    onError: (error: Error) =>
      toast({
        title: "Não foi possível registrar peso",
        description: error.message,
        variant: "destructive",
      }),
  });
  const createVaccination = useMutation({
    mutationFn: async () => {
      const { animal, clinic } = animalClinic(vaccineAnimalId);
      if (!vaccineName.trim()) throw new Error("Informe a vacina.");
      const { data: auth } = await supabase.auth.getUser();
      const { error } = await (supabase as any)
        .from("veterinary_vaccinations")
        .insert({
    ...
    onError: (error: Error) =>
      toast({
        title: "Não foi possível registrar vacina",
        description: error.message,
        variant: "destructive",
      }),
  });
  const createConsultation = useMutation({
    mutationFn: async () => {
      const animal = ((data.data?.animals ?? []) as any[]).find(
        (item) => item.id === consultationAnimalId,
      );
      if (!animal || !chiefComplaint.trim())
        throw new Error("Informe o animal e a queixa principal.");
      const clinic = (data.data?.clinics ?? []).find(
        (item) => item.id === animal.clinic_id,
      );
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["veterinary"] });
      setConsultationAnimalId("");
      setChiefComplaint("");
      setDiagnosis("");
      setPrescription("");
      toast({ title: "Consulta veterinária registrada" });
    },
    onError: (error: Error) =>
      toast({
        title: "Não foi possível registrar consulta",
        description: error.message,
        variant: "destructive",
      }),
  });
  const admitAnimal = useMutation({
    mutationFn: async () => {
      const { animal, clinic } = animalClinic(hospitalAnimalId);
      if (!boxCode.trim()) throw new Error("Informe o box de internação.");
      const occupied = ((data.data?.hospitalizations ?? []) as any[]).some(
        (stay) =>
          stay.box_code.trim().toLowerCase() === boxCode.trim().toLowerCase(),
      );
      if (occupied) throw new Error("Este box já está ocupado.");
      const { data: auth } = await supabase.auth.getUser();
      const { error } = await (supabase as any)
        .from("veterinary_hospitalizations")
        .insert({
          ...
          admission_reason: admissionReason.trim() || null,
          created_by: auth.user?.id,
          updated_by: auth.user?.id,
        });
      if (error) throw error;
    ...
    onError: (error: Error) =>
      toast({
        title: "Não foi possível registrar internação",
        description: error.message,
        variant: "destructive",
      }),
  });
  const createHospitalizationTask = useMutation({
    mutationFn: async () => {
      if (!taskHospitalizationId || !taskTitle.trim())
        throw new Error("Informe a internação e a tarefa.");
      const { data: auth } = await supabase.auth.getUser();
      const { error } = await (supabase as any)
        .from("veterinary_hospitalization_tasks")
        .insert({
          hospitalization_id: taskHospitalizationId,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["veterinary"] });
      setTaskTitle("");
      toast({ title: "Tarefa programada" });
    },
    onError: (error: Error) =>
      toast({
        title: "Não foi possível programar tarefa",
        description: error.message,
        variant: "destructive",
      }),
  });
  const finishHospitalizationTask = useMutation({
    ...
    onError: (error: Error) =>
      toast({
        title: "Não foi possível concluir tarefa",
        description: error.message,
        variant: "destructive",
      }),
  });
  const dischargeAnimal = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["veterinary"] });
      toast({ title: "Alta veterinária registrada" });
    },
    onError: (error: Error) =>
      toast({
        title: "Não foi possível registrar alta",
        description: error.message,
        variant: "destructive",
      }),
  });
  const create = useMutation({
    mutationFn: async () => {
      if (!name.trim() || !guardianId || !clinicId)
        throw new Error("Informe clínica, animal e tutor.");
      const clinic = (data.data?.clinics ?? []).find(
        (item) => item.id === clinicId,
      );
      if (!clinic) throw new Error("Clínica veterinária inválida.");
      const { data: auth } = await supabase.auth.getUser();
      const { data: animal, error } = await (supabase as any)
        .from("animals")
        .insert({
          organization_id: clinic.organization_id,
          clinic_id: clinic.id,
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Clínica veterinária</h1>
            <p className="text-sm text-muted-foreground">
              Cadastre animais e mantenha tutores vinculados ao cadastro mestre
              de pessoas.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Novo animal</CardTitle>
              <CardDescription>
                O animal fica separado na clínica veterinária selecionada.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={submit} className="space-y-3">
                ...
                <Button disabled={create.isPending || !guardianId || !clinicId}>
                  Cadastrar animal
                </Button>
              </form>
            </CardContent>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {animal.species}
                          {animal.breed ? ` · ${animal.breed}` : ""} · tutor{" "}
                          {animal.animal_guardians?.[0]?.person?.full_name ??
                            "—"}
                        </div>
                      </div>
                    </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Consulta veterinária</CardTitle>
              <CardDescription>
                Queixa, diagnóstico e prescrição vinculados ao animal.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form
              ...
              >
                <div className="space-y-1">
                  <Label>Animal</Label>
                  <Select
                    value={consultationAnimalId}
                    onValueChange={setConsultationAnimalId}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Selecione" />
                    </SelectTrigger>
              ...
              {consultations.length ? (
                consultations.map((consultation: any) => {
                  const animal = animals.find(
                    (item: any) => item.id === consultation.animal_id,
                  );
                  return (
                    <div key={consultation.id} className="rounded border p-3">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Admissão, plano de cuidados, execução e alta por box.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              >
                <Select
                  value={hospitalAnimalId}
                  onValueChange={setHospitalAnimalId}
                >
                  <SelectTrigger>
              ...
              <form
                onSubmit={(event) => {
                  event.preventDefault();
                  createHospitalizationTask.mutate();
                }}
                className="grid gap-2 border-t pt-4 sm:grid-cols-2"
              >
                <Select
                  value={taskHospitalizationId}
                  onValueChange={setTaskHospitalizationId}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Internação ativa" />
                  </SelectTrigger>
                  <SelectContent>
                    {hospitalizations.map((stay: any) => (
                      <SelectItem key={stay.id} value={stay.id}>
                        Box {stay.box_code} ·{" "}
                        {animals.find(
                          (animal: any) => animal.id === stay.animal_id,
                        )?.name ?? "Animal"}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <Input
                  ...
                  onChange={(event) => setTaskScheduledAt(event.target.value)}
                />
                <Button
                  className="sm:col-span-2"
                  variant="secondary"
                  disabled={
                    createHospitalizationTask.isPending ||
                    !taskHospitalizationId ||
                    !taskTitle.trim()
                  }
                >
                  Programar cuidado
              ...
              <div className="space-y-2 border-t pt-4">
                {hospitalizations.length ? (
                  hospitalizations.map((stay: any) => (
                    <div key={stay.id} className="rounded border p-3 text-sm">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          disabled={dischargeAnimal.isPending}
                          onClick={() => dischargeAnimal.mutate(stay.id)}
                        >
                          Dar alta
                        </Button>
                      </div>
                      <span className="mt-1 block text-xs text-muted-foreground">
                        {stay.admission_reason || "Sem motivo informado"} ·
                        desde{" "}
                        {new Date(stay.admitted_at).toLocaleString("pt-BR")}
                      </span>
                      {hospitalizationTasks
                        .filter(
                          (task: any) => task.hospitalization_id === stay.id,
                        )
                        .map((task: any) => (
                          <div
                            key={task.id}
                            className="mt-2 flex items-center justify-between gap-2 rounded bg-muted px-2 py-1 text-xs"
                          >
                            <span>
                            ...
                            <Button
                              size="sm"
                              variant="ghost"
                              disabled={finishHospitalizationTask.isPending}
                              onClick={() =>
                                finishHospitalizationTask.mutate(task.id)
                              }
                            >
                              Concluir
                            </Button>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Peso e acompanhamento</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              >
                <Select
                  value={weightAnimalId}
                  onValueChange={setWeightAnimalId}
                >
                ...
                <Button
                  disabled={
                    !weightAnimalId || !weightKg || createWeight.isPending
                  }
                >
                  Registrar peso
                </Button>
              ...
              {weights.map((item: any) => (
                <p key={item.id} className="border-t pt-2 text-sm">
                  {animals.find((animal: any) => animal.id === item.animal_id)
                    ?.name ?? "Animal"}{" "}
                  · <strong>{item.weight_kg} kg</strong> ·{" "}
                  {new Date(item.measured_at).toLocaleDateString("pt-BR")}
                </p>
              ))}
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              >
                <Select
                  value={vaccineAnimalId}
                  onValueChange={setVaccineAnimalId}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Animal" />
                  </SelectTrigger>
