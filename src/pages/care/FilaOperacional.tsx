...
export default function FilaOperacional() {
  ...
  const load = async () => {
    ...
    const { data: session, error: sessionError } = await (supabase.from("queue_sessions") as any).select("id,status,service_date").eq("clinic_id", clinic.id).eq("service_date", today).maybeSingle();
    if (sessionError) {
