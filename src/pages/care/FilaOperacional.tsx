...
type QueueSession = {
  id: string; status: "open" | "paused" | "closed"; service_date: string;
...
export default function FilaOperacional() {
  ...
  const load = async () => {
    ...
    const { data: session, error: sessionError } = await (supabase.from("queue_sessions") as any).select("id,status,service_date,entry_mode,max_capacity,concurrent_capacity,starts_at,ends_at").eq("clinic_id", clinic.id).eq("service_date", today).maybeSingle();
    if (sessionError) {
