...
serve(async (req) => {
  ...
  try {
    ...
    const { invitation_id, validade_dias } = await req.json();
    ...
    const { data, error } = await admin.from("supplier_invitations")
      .update({ status: "pendente", expires_at: expires.toISOString(), used_at: null, used_by_user_id: null })
      .eq("id", invitation_id)
      .neq("status", "usado")
      .select().maybeSingle();
    ...
    return new Response(JSON.stringify({ success: true, invitation: data, link: `${origin}/convite-fornecedor/${data.token}` }),
