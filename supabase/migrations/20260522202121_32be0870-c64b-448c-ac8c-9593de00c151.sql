...
-- Fix CI approval permission for organization admin
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE c.id = _ci_id
      ...
      AND m.role IN ('admin','gerente','organization_admin','administrador','gestor_aprovador','manager')
  )
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE c.id = _ci_id
      AND m.is_active = true
      AND m.role IN ('admin','gerente','organization_admin','administrador','gestor_aprovador','manager','compras')
  )
  OR EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.is_super_admin = true);
$function$;

-- Extend profiles with personal info used by CI Solicitante portal
